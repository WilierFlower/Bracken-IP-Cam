...
// /capture and /snapshot: single JPEG frame (Duet Web Control can poll /snapshot)
...
// /stream: MJPEG stream (Duet Web Control webcam URL); can be toggled via /control?var=stream&val=0/1
...
static esp_err_t status_handler(httpd_req_t *req) {
  ...
  int len = snprintf(json_response, sizeof(json_response),
                     "{\"framesize\":%u,\"framesize_name\":\"%s\",\"quality\":%u,\"stream_delay\":%u,\"stream_enabled\":%u}",
                     s->status.framesize, framesize_name((framesize_t)s->status.framesize),
                     s->status.quality, stream_delay_ms, stream_enabled ? 1 : 0);
